 ...
 */
template <typename T = void> struct AlignedAllocator {
     ...
     */
    template <typename U> constexpr AlignedAllocator(const AlignedAllocator<U> &other) noexcept;
     ...
     */
    template <typename U> constexpr AlignedAllocator(AlignedAllocator<U> &&other) noexcept;
     ...
     */
    template <typename U> AlignedAllocator &operator=(const AlignedAllocator<U> &other) noexcept;
     ...
     * @return A reference to this.
     */
    template <typename U> AlignedAllocator &operator=(AlignedAllocator<U> &&other) noexcept;
     ...
     * @param n The number of elements previously allocated (ignored, but required by the allocator
     * concept).
     */
 ...
 */
template <typename T> struct AlignedAllocator<T>::Deleter {
    /**
     * @brief Deallocate memory previously allocated with allocate.
     * @param p A pointer to the memory to deallocate.
     */
...
template <typename T> auto AlignedAllocator<T>::allocate(std::size_t n) -> T * {
    T *ptr{};
#ifdef _WIN32
...
template <typename T> void AlignedAllocator<T>::deallocate(T *p, std::size_t) noexcept {
#ifdef _WIN32
    _aligned_free(p);
#else
    free(p);
...
template <typename T> void AlignedAllocator<T>::Deleter::operator()(T *p) const noexcept {
