...
template <auto closeFunction> class Hdf5Handle {
    ...
    static_assert(std::is_invocable_v<decltype(closeFunction), hid_t>,
                  "closeFunction must be callable with hid_t");
...
constexpr Hdf5Handle<closeFunction>::Hdf5Handle(hid_t &&handle) : mHandle(std::move(handle)) {}
...
template <auto closeFunction> Hdf5Handle<closeFunction>::~Hdf5Handle() {
    if (mHandle != H5I_INVALID_HID) {
        closeFunction(mHandle);
    }
}

template <auto closeFunction> constexpr void Hdf5Handle<closeFunction>::reset(hid_t &&handle) {
    if (mHandle != H5I_INVALID_HID) {
        closeFunction(mHandle);
    }
...
template <auto closeFunction> constexpr hid_t Hdf5Handle<closeFunction>::release() {
    return std::exchange(mHandle, H5I_INVALID_HID);
}

template <auto closeFunction> constexpr bool Hdf5Handle<closeFunction>::valid() const {
    return mHandle != H5I_INVALID_HID;
}

template <auto closeFunction> constexpr Hdf5Handle<closeFunction>::operator hid_t() const {
    return mHandle;
}
