...
int main(int argc, char *argv[]) {
    ...
    try {
        ...
        if (simulationProps.isValidation() && rank == 0) {
            ...
            std::vector<float, AlignedAllocator<float>> absError(materialProps.getGridPointCount(),
                                                                 0.f);

            auto [ok, errorInfo] =
                verifyResults(parallelResult.data(), sequentialResult.data(),
                              materialProps.getGridPointCount(), absError.data(), 0.001f);
            ...
            if (!simulationProps.getDebugImageFileName().empty()) {
                ...
                saveAsImage(errImageName, absError.data(), materialProps.getEdgeSize(),
                            std::make_pair(0.0f, 0.001f));
            }
        }
