...
void MaterialProperties::load(std::string_view fileName, bool loadData) {
    ...
    if (loadData) {
...

        {
            ...
            H5Dread(datasetHandle, H5T_IEEE_F32LE, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                    mDomainParams.data());
        }

        {
            // 7. Read "/InitialTemperature" which contains initial temperature distribution.
            Hdf5DatasetHandle datasetHandle(
                H5Dopen(fileHandle, "/InitialTemperature", H5P_DEFAULT));
...
const std::vector<float, AlignedAllocator<float>> &
MaterialProperties::getInitialTemperature() const {
    return mInitTemp;
}

float MaterialProperties::getCoolerTemperature() const { return mCoolerTemp; }

float MaterialProperties::getHeaterTemperature() const { return mHeaterTemp; }

std::size_t MaterialProperties::getEdgeSize() const { return mEdgeSize; }

std::size_t MaterialProperties::getGridPointCount() const { return mGridPointCount; }
