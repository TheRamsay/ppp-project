...
HeatSolverBase::HeatSolverBase(const SimulationProperties &simulationProps,
    ...
    : mSimulationProps(simulationProps), mMaterialProps(materialProps) {}
...
bool HeatSolverBase::shouldPrintProgress(std::size_t iteration) const {
    ...
    return !((iteration % (nTotalIters > 10 ? (nTotalIters / 10) : 1) != 0 &&
              iteration + 1 != nTotalIters) ||
...
void HeatSolverBase::printFinalReport(double totalTime, float middleColAvgTemp) const {
    ...
    if (!mSimulationProps.isBatchMode()) {
        ...
        fmt::print("{};{};{};{};{}\n", mSimulationProps.getOutputFileName(codeType), codeType,
...
void HeatSolverBase::storeDataIntoFile(hid_t fileHandle, std::size_t iteration, const float *data) {
    ...
    {
        // Create new dataset "/Timestep_N/Temperature" which is simulation-domain sized 2D array of
        // "float"s.
        ...
        Hdf5DatasetHandle dataSetHandle(H5Dcreate(groupHandle, dataSetName.data(), H5T_NATIVE_FLOAT,
                                                  dataSpaceHandle, H5P_DEFAULT, H5P_DEFAULT,
                                                  H5P_DEFAULT));
...
void HeatSolverBase::updateTile(const float *oldTemp, float *newTemp, const float *params,
                                const int *map, std::size_t offsetX, std::size_t offsetY,
                                std::size_t sizeX, std::size_t sizeY, std::size_t strideX) const {
    const float airflowRate = mSimulationProps.getAirflowRate();
    const float coolerTemp = mMaterialProps.getCoolerTemperature();

#pragma omp parallel for firstprivate(oldTemp, newTemp, params, map, offsetX, offsetY, sizeX,      \
                                          sizeY, strideX, airflowRate, coolerTemp)
...
#pragma omp simd aligned(oldTemp, newTemp, params, map : AlignedAllocator<>::alignment)
        for (std::size_t j = offsetX; j < offsetX + sizeX; ++j) {
            computePoint(oldTemp, newTemp, params, map, i, j, strideX, airflowRate, coolerTemp);
        }
    }
}
