...
SimulationProperties::SimulationProperties()
    : mNIterations(1), mWriteIntensity(1000), mAirflowRate(0.001f), mExecution(Execution::seq),
      mDebugFlag(false), mVerificationFlag(false), mSequentialFlag(false), mBatchMode(false),
      mBatchModeHeader(false), mUseParallelIO(false), mDecomposition(Decomposition::d1) {}
...
void SimulationProperties::parseCommandLine(int argc, char *argv[]) {
    ...
    options.add_options()("m,mode",
                          ...
                          cxxopts::value<int>(), "<int>")("n,iterations", "Number of iterations",
                                                          cxxopts::value<std::size_t>(), "<uint>")(
        "i,input", "Input material file name", cxxopts::value<std::string>(),
        "file")("t,threads", "Number of OpenMP threads", cxxopts::value<int>()->default_value("1"),
                "<uint>")("o,output", "Output HDF5 file name",
        ...
        "b,batch", "Batch mode")("B,batch-with-header",
                                 "Batch mode with header")("p,parallel-io", "Parallel I/O mode")(
        "r,render", "Render results into *.png image", cxxopts::value<std::string>(),
        "<string>")("g,decomp-2d", "Use 2D decomposition instead of 1D")("h,help", "Print usage");
...
void SimulationProperties::printParameters(const MaterialProperties &materialProps) const {
    ...
    if (mBatchMode) {
        ...
        fmt::print("{};{};{};{};{};{};{};{};{};", size, mGridSize[0], mGridSize[1],
                   getThreadCount(), materialProps.getEdgeSize(), getNumIterations(),
                   getWriteIntensity(), getAirflowRate(), getMaterialFileName());
    } else {
        fmt::print(
            ...
            "Decomposition type:       {} ({}, {})\n"
            "...................................................\n",
            materialProps.getEdgeSize(), materialProps.getEdgeSize(), getNumIterations(), size,
            getThreadCount(), getWriteIntensity(), getAirflowRate(), getMaterialFileName(),
            getOutputFileName(), static_cast<std::underlying_type_t<Execution>>(mExecution),
            ((mDecomposition == Decomposition::d1) ? "1D" : "2D"), mGridSize[0], mGridSize[1]);
    }
}
...
bool SimulationProperties::isRunParallel() const {
    return (mVerificationFlag || mDebugFlag || mExecution == Execution::par_p2p ||
            mExecution == Execution::par_rma);
}

bool SimulationProperties::isRunParallelP2P() const { return (mExecution == Execution::par_p2p); }

bool SimulationProperties::isRunParallelRMA() const { return (mExecution == Execution::par_rma); }

bool SimulationProperties::isValidation() const { return (mVerificationFlag || mDebugFlag); }

std::size_t SimulationProperties::getNumIterations() const { return mNIterations; }

int SimulationProperties::getThreadCount() const { return mThreadCount; }

std::size_t SimulationProperties::getWriteIntensity() const { return mWriteIntensity; }

float SimulationProperties::getAirflowRate() const { return mAirflowRate; }

std::string_view SimulationProperties::getMaterialFileName() const { return mMaterialFileName; }
...
std::string_view SimulationProperties::getDebugImageFileName() const { return mDebugImageName; }

bool SimulationProperties::isBatchMode() const { return mBatchMode; }

bool SimulationProperties::useParallelIO() const { return mUseParallelIO; }

bool SimulationProperties::isDebug() const { return mDebugFlag; }
