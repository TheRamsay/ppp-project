...
void saveAsImage(std::string_view fileName, const float *array, std::size_t edgeSize,
    ...
    for (unsigned i = 0; i < edgeSize; ++i) {
        ...
        for (unsigned j = 0; j < edgeSize; ++j) {
            ...
            float red = (normalValue < 0.5f) ? (-6.0f * normalValue + 67.0f / 32.0f)
                                             : (6.0f * normalValue - 79.0f / 16.0f);
            float green = (normalValue < 0.4f) ? (6.0f * normalValue - 3.0f / 32.0f)
                                               : (-6.0f * normalValue + 79.0f / 16.0f);
            float blue = (normalValue < 0.7f) ? (6.0f * normalValue - 67.0f / 32.0f)
                                              : (6.0f * normalValue + 195.0f / 32.0f);

            imageData[3 * dstIdx + 0] =
                static_cast<std::uint8_t>(std::clamp(red, 0.f, 1.f) * 255.0f);
            imageData[3 * dstIdx + 1] =
                static_cast<std::uint8_t>(std::clamp(green, 0.f, 1.f) * 255.0f);
            imageData[3 * dstIdx + 2] =
                static_cast<std::uint8_t>(std::clamp(blue, 0.f, 1.f) * 255.0f);
        }
        ...
        for (unsigned j = 2; j < paletteWidth; ++j) {
            ...
            float normalValue =
                1.0f - static_cast<float>(i) * (1.0f / static_cast<float>(edgeSize));
    ...
    stbi_write_png(fileName.data(), static_cast<int>(edgeSize + paletteWidth),
                   static_cast<int>(edgeSize), 3, imageData.data(),
                   static_cast<int>(3 * (edgeSize + paletteWidth)));
}

std::pair<bool, ErrorInfo> verifyResults(const float *result, const float *refResult,
                                         std::size_t gridPointCount, float *outAbsDiff,
                                         float epsilon) {
    ErrorInfo errorInfo{};
