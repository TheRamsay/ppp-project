...
srun $BINARY_PATH -B    -n 100 -t $OMP_NUM_THREADS -m 1 -w $DISK_WRITE_INTENSITY -i input_data_1024.h5 >> $STDOUT_FILE 2>> $STDERR_FILE
srun $BINARY_PATH -b -g -n 100 -t $OMP_NUM_THREADS -m 1 -w $DISK_WRITE_INTENSITY -i input_data_1024.h5 >> $STDOUT_FILE 2>> $STDERR_FILE
